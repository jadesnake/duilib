...
namespace DuiLib
{
	...
	//
	//
	DWORD __stdcall OEM2ASCII(const wchar_t *p,std::string &strOut)
	{
		if( p == 0 )	return -1;
		DWORD	dwLen = 0,dwRetIf = wcslen(p);;
		char* lpOut = 0;
		dwLen = WideCharToMultiByte(CP_ACP,0,p,dwRetIf,0,0,0,0);
		if( dwLen == 0)	return GetLastError();
		dwLen = dwLen * sizeof(char);
		lpOut = (char*)malloc(dwLen+1);
		if(lpOut)
		{
			memset(lpOut,0,dwLen+1);
			if( 0 == WideCharToMultiByte(CP_ACP,0,p,dwRetIf,lpOut,dwLen,0,0))
			{
				free(lpOut);
				lpOut = 0;
				return GetLastError();
