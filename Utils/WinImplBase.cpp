...
WindowImplBase::WindowImplBase()
	:m_bHtCap(false),m_pShadowWnd(NULL)
{
...
LRESULT WindowImplBase::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	::SetWindowPos(*this, NULL, rcClient.left, rcClient.top, rcClient.right - rcClient.left, \
		rcClient.bottom - rcClient.top, SWP_NOACTIVATE|SWP_FRAMECHANGED);
...
LRESULT WindowImplBase::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	LRESULT lRes = 0;
	BOOL bHandled = TRUE;
	if( uMsg==WM_NCLBUTTONDOWN  && wParam == HTCAPTION && m_PaintManager.IsDisabledAutoMax() )
	{
		LONG gwl_style = ::GetWindowLong(*this,GWL_STYLE);
		gwl_style &= ~WS_THICKFRAME;
		::SetWindowLong(*this, GWL_STYLE, gwl_style);
		m_bHtCap = true;
	}

	//由于无法正常收到WM_NCLBUTTONUP消息，故利用WM_NCHITTEST解决
	if( uMsg==WM_NCHITTEST && m_bHtCap && m_PaintManager.IsDisabledAutoMax() )
	{
		LONG gwl_style = ::GetWindowLong(*this,GWL_STYLE);
		gwl_style |= WS_THICKFRAME;
		::SetWindowLong(*this, GWL_STYLE, gwl_style);
	...
	if (m_pShadowWnd)
	{
		m_pShadowWnd->BeforeOwnerHandleMessage(uMsg,wParam,lParam);
	}
	switch (uMsg)
	{
	...
	if (m_pShadowWnd)
	{
		m_pShadowWnd->AfterOwnerHandleMessage(uMsg,wParam,lParam);
	}
	...
	if (m_PaintManager.MessageHandler(uMsg, wParam, lParam, lRes))
	{
		if (m_pShadowWnd)
		{
			m_pShadowWnd->AfterOwnerPaintManager(uMsg,wParam,lParam);
		}
		return lRes;
	}
...
void WindowImplBase::HandleShadowWnd()
{
	if (!m_PaintManager.GetShadowXml().IsEmpty())
	{
		m_pShadowWnd = new CShadowWnd(&m_PaintManager);
		m_pShadowWnd->Create(NULL, _T("RLShadowWindow"), 
			WS_DISABLED,WS_EX_TOOLWINDOW|WS_EX_LAYERED );
		m_pShadowWnd->ShowWindow(false);
	}
}

HWND WindowImplBase::GetShadowWnd()
{
	if (m_pShadowWnd)
	{
		return m_pShadowWnd->GetHWND();
	}
	else
	{
		return NULL;
