...
CStdStringPtrMap CPaintManagerUI::SkinColorHash;
std::map<CDuiString,long> CPaintManagerUI::mapConString;
IControlLogFunc* CPaintManagerUI::m_pConLogFunc=NULL;
...
m_bNoFocus(false)
{
	...
	::ZeroMemory(&m_rcShadow, sizeof(m_rcShadow));
}
...
CPaintManagerUI::~CPaintManagerUI()
{
	...
	if( m_hUpdateRectPen != NULL ) ::DeleteObject(m_hUpdateRectPen);

	FinalReleasePaintDC();
	RemoveFromMsgLoop();
}

void CPaintManagerUI::RemoveFromMsgLoop()
{
	m_aPreMessages.Remove(m_aPreMessages.Find(this));
}
void CPaintManagerUI::EnableNoFocus(bool  bV)
{
	m_bNoFocus = bV;
}
void CPaintManagerUI::Init(HWND hWnd)
{
	if (m_hDcPaint != NULL)
	{
		OutputDebugStr(_T("should not init again"));
		ASSERT(FALSE);
		return;
	}
...
void CPaintManagerUI::FinalReleasePaintDC()
{
	if (m_hDcPaint)
	{
...
TOOLINFO *CPaintManagerUI::GetToolInfo()
{
	return &m_ToolTip;
}
...
bool CPaintManagerUI::IsDisabledAutoMax()
{
	return m_bDisableAutoMax;
}

void CPaintManagerUI::SetDisabledAutoMax(bool bV)
{
	m_bDisableAutoMax = bV;
}

bool CPaintManagerUI::AddLogControl(LPCTSTR controlName)
{
	if(mapConString.find(controlName)==mapConString.end())
	{
		mapConString.insert(map<CDuiString,long>::value_type(controlName,0));
		return true;
	}
	else return false;
}

bool CPaintManagerUI::LogControlFunc()
{
	if(m_pConLogFunc)
	{
		map<CDuiString,long>::iterator iter = mapConString.begin();
		for (;iter!=mapConString.end();iter++)
		{
			m_pConLogFunc->LogControlCount(iter->first,iter->second,NULL);
		}
		return true;
	}
	return false;
...
bool CPaintManagerUI::RegisterLogFunc(IControlLogFunc* pCallback)
{
	m_pConLogFunc=pCallback;
	return true;
}

void CPaintManagerUI::ControlHitCount( CDuiString controlName )
{
	map<CDuiString,long>::iterator iter=mapConString.find(controlName);
	if(iter!=mapConString.end())
	{
		iter->second++;
	}
}
...
void CPaintManagerUI::RegisterUnNotifyEvent(CControlUI *pCtrl)
{
	if( m_UnEventNotifyCtrl.Find(pCtrl) < 0 )
	{
		m_UnEventNotifyCtrl.Add(pCtrl);
	}
}

//反注册接收duilib未通知到事件 Fixed by jiayh
void CPaintManagerUI::UnRegisterUnNotifyEvent(CControlUI *pCtrl)
{
	int iIndex = m_UnEventNotifyCtrl.Find(pCtrl);
	if( iIndex != -1 )
	{
		m_UnEventNotifyCtrl.Remove(iIndex);
	}
}
...
//#endif
    ...
    case WM_APP + 1:
        {
			...
			{
				if(m_pEventClick==m_aDelayedCleanup[i])
					m_pEventClick = NULL;
				if(m_pEventHover==m_aDelayedCleanup[i])
					m_pEventHover = NULL;
				if(m_pFocus==m_aDelayedCleanup[i])
					m_pFocus = NULL;
    ...
    case WM_CLOSE:
        {
			...
			if( hwndParent != NULL && m_bNoFocus==false) {
				::SetFocus(hwndParent);
			}
        }
        break;
    ...
    case WM_PAINT:
        {
			...
			if( m_bLayered )
			{
				...
				{
					//layer优先使用gdi+ 32图，如果分配失败使用dib图
					Gdiplus::Bitmap bitmap(rcClient.right - rcClient.left,rcClient.bottom - rcClient.top);
					bitmap.GetHBITMAP(Gdiplus::Color(0,0,0,0),&m_hbmpBackground);
					m_hDcBackground  = ::CreateCompatibleDC(m_hDcPaint);
					if( !m_hbmpBackground )
					{
						m_hbmpBackground = ::CreateCompatibleBitmap(m_hDcPaint, 
            ...
            if( m_bOffscreenPaint )
            {
                ...
                if( m_bAlphaBackground )
				{
                    ...
                    if( m_hbmpBackground == NULL )
					{
                        ...
                        ASSERT(m_hDcBackground);
                        ASSERT(m_hbmpBackground);
                        hOldBg = (HBITMAP)::SelectObject(m_hDcBackground, m_hbmpBackground);
					...
					::SelectObject(m_hDcBackground, hOldBg);
                }
    ...
    case WM_MOUSEHOVER:
        ...
        return true;
	case WM_KILLFOCUS:	//fixed by jiayh
		{
			if( m_hwndTooltip != NULL ) 
    ...
    case WM_MOUSELEAVE:
        {
            ...
            m_bMouseTracking = false;
			for( int j = 0; j < m_UnEventNotifyCtrl.GetSize(); j++ ) 
			{
				TEventUI event = { 0 };
				event.dwTimestamp = ::GetTickCount();
				event.Type = UIEVENT_MOUSELEAVE;
				event.pSender = static_cast<CControlUI*>(m_UnEventNotifyCtrl[j]);
				event.pSender->Event(event);
				if( m_UnEventNotifyCtrl.GetSize() == 0 )
					break;
			}
		}
        break;
    case WM_MOUSEMOVE:
        {
            ...
            event.ptMouse = pt;
            event.dwTimestamp = ::GetTickCount();
			event.wParam  = wParam;	//fixed by jiayh
    ...
    case WM_LBUTTONDOWN:
        {
			...
			//控件点击计数
			ControlHitCount(pControl->GetName());
            pControl->SetFocus();
            SetCapture();
            TEventUI event = { 0 };
...
void CPaintManagerUI::LoadSkinColor()
{
	CMarkup xmldoc;
	CStdString name;
	xmldoc.LoadFromFile(_T("SkinColor.xml"));
	CMarkupNode pNode = xmldoc.GetRoot();
	for( CMarkupNode node = pNode.GetChild() ; node.IsValid(); node = node.GetSibling() ) 
	{
		name = node.GetName();
		name+=node.GetAttributeName(0);
		CStdString* value=new CStdString;
		value->Assign(node.GetAttributeValue(0));
		CPaintManagerUI::SkinColorHash.Insert(name,(void*)value);
	}
}

void CPaintManagerUI::SetShadowXml(LPCTSTR pstrShadowXml)
{
	m_strShadowXml = pstrShadowXml;
}

CDuiString CPaintManagerUI::GetShadowXml()
{
	return m_strShadowXml;
}

void CPaintManagerUI::SetShadowPadding(RECT rcShadow)
{
	m_rcShadow = rcShadow;
}

RECT CPaintManagerUI::GetShadowPadding()
{
	return m_rcShadow;
}
