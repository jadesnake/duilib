...
void	CControlUI::SetLeftBorderColor(DWORD dwV)
{
	if( m_dwBorderColor_L == dwV )	return;
	m_dwBorderColor_L = dwV;
	Invalidate();
}
void	CControlUI::SetRightBorderColor(DWORD dwV)
{
	if( m_dwBorderColor_R == dwV )	return;
	m_dwBorderColor_R = dwV;
	Invalidate();
}
void	CControlUI::SetTopBorderColor(DWORD dwV)
{
	if( m_dwBorderColor_T == dwV )	return;
	m_dwBorderColor_T = dwV;
	Invalidate();
}
void	CControlUI::SetBottomBorderColor(DWORD dwV)
{
	if( m_dwBorderColor_B == dwV )	return;
	m_dwBorderColor_B = dwV;
	Invalidate();
}
DWORD	CControlUI::GetLeftBorderColor()
{
	return m_dwBorderColor_L;
}
DWORD	CControlUI::GetRightBorderColor()
{
	return m_dwBorderColor_R;
}
DWORD	CControlUI::GetTopBorderColor()
{
	return m_dwBorderColor_T;
}
DWORD	CControlUI::GetBottomBorderColor()
{
	return m_dwBorderColor_B;
}
...
void CControlUI::SetFixedWidth(int cx,bool bUpdate)
{
    if( cx < 0 ) return; 
    m_cxyFixed.cx = cx;
	if( bUpdate )
...
void CControlUI::SetFixedHeight(int cy,bool bUpdate)
{
    if( cy < 0 ) return; 
    m_cxyFixed.cy = cy;
	if( bUpdate )
...
bool CControlUI::IsSetVisible() const
{
	return m_bVisible;
}
...
{
    if( _tcscmp(pstrName,_T("bordercolorL")) == 0 )
	{
		if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
		LPTSTR pstr = NULL;
		SetLeftBorderColor(_tcstoul(pstrValue, &pstr, 16));
	}
	else if( _tcscmp(pstrName,_T("bordercolorR")) == 0 )
	{
		if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
		LPTSTR pstr = NULL;
		SetRightBorderColor(_tcstoul(pstrValue, &pstr, 16));
	}
	else if( _tcscmp(pstrName,_T("bordercolorT")) == 0 )
	{
		if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
		LPTSTR pstr = NULL;
		SetTopBorderColor(_tcstoul(pstrValue, &pstr, 16));
	}
	else if( _tcscmp(pstrName,_T("bordercolorB")) == 0 )
	{
		if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
		LPTSTR pstr = NULL;
		SetBottomBorderColor(_tcstoul(pstrValue, &pstr, 16));
	}
		...
		else
		{
			...
			else if(m_rcBorderSize.left > 0 || m_rcBorderSize.top > 0 || m_rcBorderSize.right > 0 || m_rcBorderSize.bottom > 0)
			{
				...
				if( m_rcBorderSize.left > 0 )
				{
					...
					if( m_dwBorderColor_L )
					{
						CRenderEngine::DrawLine(hDC,rcBorder,m_rcBorderSize.left,
							GetAdjustColor(m_dwBorderColor_L),m_nBorderStyle);
					}
					else
					{
						CRenderEngine::DrawLine(hDC,rcBorder,m_rcBorderSize.left,
				...
				if(m_rcBorderSize.top > 0)
				{
					...
					if( m_dwBorderColor_T )
					{
						CRenderEngine::DrawLine(hDC,rcBorder,m_rcBorderSize.top,
							GetAdjustColor(m_dwBorderColor_T),m_nBorderStyle);
					}
					else
					{
						CRenderEngine::DrawLine(hDC,rcBorder,m_rcBorderSize.top,
				...
				{
					rcBorder		= m_rcItem;
					rcBorder.top	= m_rcItem.bottom - m_rcBorderSize.bottom;
					rcBorder.bottom = rcBorder.top;
					if( m_dwBorderColor_B )
					{
						CRenderEngine::DrawLine(hDC,rcBorder,m_rcBorderSize.bottom,
							GetAdjustColor(m_dwBorderColor_B),m_nBorderStyle);
					}
					else
					{
						CRenderEngine::DrawLine(hDC,rcBorder,m_rcBorderSize.bottom,
