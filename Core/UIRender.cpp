...
//Layered window text supporting
HBITMAP CreateAlphaTextBitmap(LPCTSTR inText, HFONT inFont, COLORREF inColour)
{ 
...
// when using DrawText with UpdateLayeredWindow, all text will become
// transparent, then we will need this function instead of DrawText to solve
// this problem.
void DrawTextUnderLayered(HDC inDC, LPCTSTR szText, HFONT hFont, DWORD dwColor, RECT rc)
{
	HBITMAP MyBMP = CreateAlphaTextBitmap(szText, hFont, dwColor); 
	...
	if (MyBMP)
	{
		...
		if (hOldBMP)
		{
			...
			AlphaBlend(inDC, rc.left, rc.top, BMInf.bmWidth, BMInf.bmHeight, hTempDC, 0, 0, BMInf.bmWidth, BMInf.bmHeight, bf); 
...
void CRenderEngine::DrawGradient(HDC hDC, const RECT& rc, DWORD dwFirst, DWORD dwSecond, bool bVertical, int nSteps)
{
	...
	GradientFill(hPaintDC, triv, 2, &grc, 1, bVertical ? GRADIENT_FILL_RECT_V : GRADIENT_FILL_RECT_H);

    if( bAlpha < 255 ) {
        BLENDFUNCTION bf = { AC_SRC_OVER, 0, bAlpha, AC_SRC_ALPHA };
        AlphaBlend(hDC, rc.left, rc.top, cx, cy, hPaintDC, 0, 0, cx, cy, bf);
...
void CRenderEngine::DrawText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrText, DWORD dwTextColor, int iFont, UINT uStyle)
{
	...
	{
		DrawTextUnderLayered(hDC, pstrText, pManager->GetFont(iFont), dwTextColor, rc);
	}
	else
	{
