        ...
        if( _tcscmp(pstrClass, _T("Window")) == 0 ) {
            if( pManager->GetPaintWindow() ) {
                ...
                for( int i = 0; i < nAttributes; i++ ) {
                    ...
                    if( _tcscmp(pstrName, _T("size")) == 0 ) {
					...
					else if( _tcscmp(pstrName,_T("disabledautomax")) == 0 ){
						if( 0 == _tcscmp(pstrValue,_T("true")) )
							pManager->SetDisabledAutoMax(true);
						else
							pManager->SetDisabledAutoMax(false);
					}
						...
						pManager->SetShadowPadding(rcPadding);
					}
					else if (_tcscmp(pstrName,_T("shadowxml")) == 0)
					{
						pManager->SetShadowXml(pstrValue);
					}
