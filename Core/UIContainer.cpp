...
namespace DuiLib
{
	...
	CContainerUI::CContainerUI()
		...
		m_bScrollProcess_V(false),
		m_bScrollProcess_H(false),
		m_bUsedOuterVScrollBar(false)
	{
	...
	//添加且不刷新界面 - 可防止设置控件位置时，Duilib捣乱（重新计算所有控件的位置）
	bool CContainerUI::AddEx(CControlUI* pControl,bool bUpdate/* = true*/)
	{
		if( pControl == NULL) return false;

		if( m_pManager != NULL ) m_pManager->InitControls(pControl, this);
		if( IsVisible() && bUpdate ) 
			NeedUpdate();
		else 
		{
			if (!IsVisible())
	...
	bool CContainerUI::Remove(CControlUI* pControl)
	{
		...
		for( int it = 0; it < m_items.GetSize(); it++ ) {
			CControlUI *ui = static_cast<CControlUI*>(m_items[it]);
			if( ui == pControl ) {
				NeedUpdate();
				if( m_bAutoDestroy ) {
	...
	//移除且不刷新界面 - 可防止设置控件位置时，Duilib捣乱（重新计算所有控件的位置）
	bool CContainerUI::RemoveEx(CControlUI* pControl,bool bUpdate/* = true*/)
	{
		if( pControl == NULL) return false;
	...
	void CContainerUI::RemoveAll()
	{
		for( int it = 0; m_bAutoDestroy && it < m_items.GetSize(); it++ ) {
			CControlUI *ui = static_cast<CControlUI*>(m_items[it]);
			if( m_bDelayedDestroy && m_pManager )
				m_pManager->AddDelayedCleanup(ui);             
			else if(m_pManager)	{
				m_pManager->ReapObjects(ui);
				delete ui;
			}
	...
	CScrollBarUI* CContainerUI::GetScrollBarUI()
	{
	...
	void CContainerUI::EnableScrollBar(bool bEnableVertical, bool bEnableHorizontal)
	{
		if( bEnableVertical && !m_pVerticalScrollBar ) {
			m_pVerticalScrollBar = GetScrollBarUI();	//modify by jiayh
		...
		if( bEnableHorizontal && !m_pHorizontalScrollBar ) {
			m_pHorizontalScrollBar = GetScrollBarUI();	//modify by jiayh
	...
	void CContainerUI::ProcessScrollBar(RECT rc, int cxRequired, int cyRequired)
	{
		if( m_pHorizontalScrollBar )
		{
			...
			if( cxScroll <= 0 && !m_bScrollProcess_H) 
			{
		...
		if( cyRequired > rc.bottom - rc.top && !m_pVerticalScrollBar->IsVisible() ) {
			...
			m_bScrollProcess_V = true;
			SetPos(m_rcItem);
			m_bScrollProcess_V = false;
			
			if( UsedOuterVScrollBar() )
			{
		...
		if( cyScroll <= 0 && !m_bScrollProcess_V) {
		...
		{
			//if the container used outer scrollbar, then just let the outer scrollbar control stay where it was
			if (UsedOuterVScrollBar() == false)
			{
	...
	void CContainerUI::SetVerticalScrollBar( CScrollBarUI* pScrollBar )
	{
		...
		m_bUsedOuterVScrollBar = true;
	}

	bool CContainerUI::UsedOuterVScrollBar()
	{
		return m_bUsedOuterVScrollBar;
	}
} // namespace DuiLib
