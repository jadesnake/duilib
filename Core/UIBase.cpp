...
CDuiString DUI__TraceMsg(UINT uMsg)
{
...
//1是否包含2
bool IsInsetRect(RECT *rc1,RECT *rc2)
{
	POINT lpt={rc2->left,rc2->top};
	POINT Rpt={rc2->right,rc2->bottom};
	return (::PtInRect(rc1,lpt) && ::PtInRect(rc1,Rpt));
}
BOOL CALLBACK SaveModalProc(HWND hwnd,LPARAM lParam)
{
	CStdPtrArray *array = (CStdPtrArray*)(void*)lParam;
...
BOOL CALLBACK ChildModalProc(HWND hwnd,LPARAM lParam)
{
	CStdPtrArray *array = (CStdPtrArray*)(void*)lParam;
	if( ::IsWindowVisible(hwnd) && ::IsWindowEnabled(hwnd) )
	{
...
HWND FindNxtWindow(HWND hCurWin)
{
	HWND hRet =NULL;
	TCHAR chClass[256];
	ZeroMemory(chClass,sizeof(chClass));
	hRet = GetNextWindow(hCurWin,GW_HWNDNEXT);
	if( hRet )
	{
		RealGetWindowClass(hRet,chClass,256);
		if( 0==_tcscmp(_T("ShadowWnd"),chClass) )
		{
			return FindNxtWindow(hRet);
		}
	}
	return hRet;
}
BOOL CALLBACK SaveChildProc(HWND hwnd,LPARAM lParam)
{
	CStdPtrArray *array = (CStdPtrArray*)(void*)lParam;
...
{
	CStdPtrArray lstNxtWin;
	//CStdPtrArray lstChildWin;
	...
	if( hPreFocus==NULL )
	{
		hPreFocus = FindNxtWindow(m_hWnd);
	}
	{
		//将本线程其他顶层窗口锁定
		::EnumThreadWindows(GetCurrentThreadId(),SaveModalProc,(LPARAM)(void*)&lstNxtWin);
		int nSelf = lstNxtWin.Find(m_hWnd);
		lstNxtWin.Remove(nSelf);
		for( int nI=0;nI < lstNxtWin.GetSize();nI++ )
		{
			HWND hNxtWin = (HWND)(void*)lstNxtWin.GetAt(nI);
			::EnableWindow(hNxtWin,FALSE);
		}
	}
	{
		//枚举子窗口
		//::EnumChildWindows(m_hWnd,SaveChildProc,(LPARAM)(void*)&lstChildWin);
	}
	MSG msg = { 0 };
	while( ::GetMessage(&msg, NULL, 0, 0) ) {
        if( msg.hwnd == m_hWnd && msg.message==WM_CLOSE )
		{
            nRet = m_nCloseVal;	//msg.wParam;
			bClose = true;
        }
		//模拟windows系统模态窗口的效果：鼠标击窗口之外时让任务栏闪
		//为了防止子窗口在别的线程运行所做的特殊处理
		/*if( msg.message==WM_KEYDOWN || msg.message==WM_KEYUP  )
		{
			HWND hIsChild=NULL;
			if( lstChildWin.GetSize() )
			{
				for(int nI=0;nI < lstChildWin.GetSize();nI++)
				{
					HWND hC = (HWND)(void*)lstChildWin.GetAt(nI);
					if( hC == msg.hwnd )
					{
						hIsChild = hC;
						break;
					}
					else if( ::IsChild(hC,msg.hwnd) )
					{
						hIsChild = msg.hwnd;
						break;
					}
			...
			else if( ::IsChild(m_hWnd,msg.hwnd) )
			{
				hIsChild = msg.hwnd;
			}
			if( hIsChild )
		...
		if( msg.message == WM_LBUTTONDOWN   || msg.message==WM_MBUTTONDBLCLK	||
			msg.message == WM_RBUTTONDOWN   || msg.message==WM_NCHITTEST		||
...
LRESULT CALLBACK CWindowWnd::__WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if( uMsg == WM_NCCREATE ) {
        ...
        if( uMsg == WM_NCDESTROY && pThis != NULL ) {
            ...
            HWND htemp = pThis->m_hWnd;
			pThis->m_hWnd = NULL;
			pThis->OnFinalMessage(hWnd);	//pThis will be deleted in this call
			::DestroyWindow(htemp);	//DestroyWindow before OnFinalMessage will cause ReleaseDC fail
            return lRes;
        }
    }
    if( pThis != NULL ) {
