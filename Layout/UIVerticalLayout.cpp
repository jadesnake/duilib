...
namespace DuiLib
{
		...
		if( m_bAutoFixed && GetCount() )
		{
			...
			if( pB && pE && (pB != pE) )
			{
				if( pB->IsVisible() && pE->IsVisible() )
				{
					RECT rcB = pB->GetPos();
					RECT rcE = pE->GetPos();
					long lH  = nBorder+rcE.bottom-rcB.top+pE->GetPadding().bottom+pB->GetPadding().top;
			...
			else if( pB )
			{
				...
				{
					RECT rcB = pB->GetPos();
					long lH  = nBorder+rcB.bottom-rcB.top+pB->GetPadding().bottom+pB->GetPadding().top;
					if( GetFixedHeight() != lH )
						SetFixedHeight(lH);
				}
				else if( GetFixedHeight() )
			...
			else if( pE )
			{
				...
				{
					RECT rcE = pE->GetPos();
					long lH  = nBorder+rcE.bottom-rcE.top+pE->GetPadding().bottom+pE->GetPadding().top;
					if( GetFixedHeight() != lH )
						SetFixedHeight(lH);
				}
				else if( GetFixedHeight() )
	...
	void CVerticalLayoutUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if( _tcscmp(pstrName, _T("sepheight")) == 0 ) SetSepHeight(_ttoi(pstrValue));
