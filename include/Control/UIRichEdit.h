...
public:
	...
	{
		IN_TYPE		nMsg;		//
		int			nBarType;	//SB_VERT ,SB_HORZ,SB_BOTH
		union
		{
			int  nMovePos;
			int  nNewRange; 
    ...
    int DeleteText(long nStartChar, long nEndChar);
	...
	bool IsPassword();
	bool IsHideCaret();
	void HideCaret(bool bHide);
	void GetCaretSize(SIZE &sz);
	//企业通兼容
	void SetFocus();
	void KillFocus();
	
	bool EnableViewChanged(bool bChanged);
	void EnableThrowScrollEvent(bool bThrow);
	SIZE GetScrollMove();
	void RemoveAll();

	CEventSource OnScrollBar;
	CEventSource OnChangeRc;
	//tom扩展接口
	ITextRange2*	GetRange(long nStartChar, long nEndChar);
	ITextDocument2*	GetDoc2();
	bool Add(CControlUI* pControl);
	bool AddAt(CControlUI* pControl, int iIndex);
	void UpdateItems();
protected:
	void	FireShowOrHideScrollbar();
