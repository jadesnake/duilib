#if !defined(__CLONE_FROM_GOOGLE_CHROME_IMEINPUT_H_)
#define __CLONE_FROM_GOOGLE_CHROME_IMEINPUT_H_
...
public:
	ImeInput();
	virtual ~ImeInput();
	void StoreDefaultCharFormat( const CHARFORMAT2 &format )
	{
		memcpy(&m_DefaultCF,&format,sizeof(CHARFORMAT2));
	}
	CHARFORMAT2&	GetDefaultCharFormat()
	{
		return m_DefaultCF;
	}
	...
	LRESULT SetImeWindowStyle(HWND window_handle, UINT message,WPARAM wparam, LPARAM lparam,bool* handled);
	...
	//   message handlers.
	bool GetResult(HWND window_handle, LPARAM lparam,std::wstring* result);
	...
	//   This function is designed for being called from WM_IME_COMPOSITION
	//   message handlers.
	bool GetComposition(HWND window_handle, LPARAM lparam,CompositionText* composition);
	...
	void UpdateCaretRect(HWND window_handle, const RECT& caret_rect);
	...
	inline bool IsUseCompositionWindow()
	{
		return use_composition_window_;
	}
	...
	std::wstring GetInputLanguageName() const;
	// Returns the text direction of the current input language.
	//1-RIGHT_TO_LEFT;2-LEFT_TO_RIGHT
	...
	static bool IsCtrlShiftPressed(int* direction);
	static COLORREF	GetDefaultColor()
	{
		return RGB(0,0,255);
	}
	void CompleteComposition(HWND window_handle);
	inline long GetCaretWidth()
	{
		return (caret_rect_.right-caret_rect_.left);
	}
	inline long GetCaretHeight()
	{
		return (caret_rect_.bottom-caret_rect_.top);
	}
	inline RECT GetCaretRECT()
	{
		return caret_rect_;
	}
	inline BOOL GetDisplayCharactor()
	{
		return m_bDisplayCharactor;
	}
	inline void SetDisplayCharactor( bool bVal )
	{
		m_bDisplayCharactor = bVal;
	}
protected:
	// Retrieves the composition information.
	void GetCompositionInfo(HIMC imm_context, LPARAM lparam,CompositionText* composition);
	// Updates the position of the IME windows.
	...
	bool GetString(HIMC imm_context, WPARAM lparam, int type,std::wstring* result);
private:
	...
	//默认字符格式
	CHARFORMAT2 m_DefaultCF;
	//
	bool m_bDisplayCharactor;		//显示中间字符
};

}

#endif //__CLONE_FROM_GOOGLE_CHROME_IMEINPUT_H_