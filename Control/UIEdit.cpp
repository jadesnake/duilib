...
namespace DuiLib
{
	...
	void CEditWnd::Init(CEditUI* pOwner)
	{
		...
		Create(m_pOwner->GetManager()->GetPaintWindow(), NULL, uStyle, 0, m_rcCal);
		HFONT hFont=NULL;
		...
		::SetForegroundWindow(m_hWnd);	//fixed by jiayh
		::SetFocus(m_hWnd);
		m_bInit = true;
	}

	void CEditWnd::CalPos()
	{
		m_rcCal.left  = m_pOwner->GetPos().left;
		m_rcCal.right = m_pOwner->GetPos().right;
		m_rcCal.top   = m_pOwner->GetPos().top;
		m_rcCal.bottom= m_pOwner->GetPos().bottom;
		//fixed by jade
		int rBorder = m_pOwner->GetBorderSize() + m_pOwner->GetRightBorderSize();
		int lBorder = m_pOwner->GetBorderSize() + m_pOwner->GetLeftBorderSize();
		int tBorder = m_pOwner->GetBorderSize() + m_pOwner->GetTopBorderSize();
		int bBorder = m_pOwner->GetBorderSize() + m_pOwner->GetBottomBorderSize();

		RECT rcInset = m_pOwner->GetTextPadding();
		m_rcCal.left += max(rcInset.left, lBorder);
		m_rcCal.right -= max(rcInset.right, rBorder);
		LONG lEditHeight = m_pOwner->GetManager()->GetFontInfo(m_pOwner->GetFont())->tm.tmHeight;
		if( lEditHeight < m_rcCal.GetHeight() ) {
			m_rcCal.top += (m_rcCal.GetHeight() - lEditHeight) / 2;
			m_rcCal.bottom = m_rcCal.top + lEditHeight;
		}
	}
	...
	LRESULT CEditWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		{
			m_pOwner->OnEditKeyDown(wParam);
			if (TCHAR(wParam) == VK_RETURN)
			{
				m_pOwner->GetManager()->SendNotify(m_pOwner, DUI_MSGTYPE_RETURN,wParam,lParam);

				TEventUI tEvent = { 0 };
				tEvent.pSender = m_pOwner;
				tEvent.Type = UIEVENT_EDIT_KEYDOWN_RETURN;
				m_pOwner->OnEdit_KeyDown(&tEvent);
			}
			else
			{
				bHandled = FALSE;
		...
		if( uMsg == WM_CHAR )
		{
			if( (!bHandled) && m_pOwner->GetEnableFloat() )
			{
				if( '0' <= TCHAR(wParam) && TCHAR(wParam) <= '9' )
				{
					CDuiString strTmp = m_pOwner->GetText();
					int nPos = strTmp.Find('.');
					if( (-1!=nPos) && (nPos!=0) )
					{
						strTmp = strTmp.Mid(0,nPos);
						if( strTmp.GetLength() >= 10 )
							bHandled = true;
					}
					else if( (nPos == -1) && (strTmp.GetLength() >= 10) )
						bHandled = true;
					bHandled = false;
				}
				else if( TCHAR(wParam)==VK_BACK || wParam==22 || wParam==24 || wParam==26 || wParam==3 )
					bHandled = false;
				else if( '.' == TCHAR(wParam) )
				{
					CDuiString strTmp = m_pOwner->GetText();
					if( -1 != strTmp.Find(TCHAR(wParam)) )
						bHandled = true;
				}
				else
					bHandled = true;
			}
			CheckInput *pHandler = m_pOwner->GetChecker();
			if( pHandler )
			{
				if( (!bHandled) && pHandler->UserInput(TCHAR(wParam)) )
					return CWindowWnd::HandleMessage(uMsg, wParam, lParam);
				else
					return lRes;
			}
	...
	LRESULT CEditWnd::OnKillFocus(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		if( m_pOwner->GetEnableFloat() )
		{
			CDuiString strTmp = m_pOwner->GetText();
			int nPos = strTmp.Find(TCHAR('.'));
			int nLen = strTmp.GetLength();
			if( (nPos == 0) && (1==nLen) )
			{
				strTmp = CDuiString(_T("0.00"));
				m_pOwner->SetText(strTmp);
			}
			else if( nPos == 0 )
			{
				strTmp = CDuiString(_T("0")) + strTmp;
				m_pOwner->SetText(strTmp);
			}
			else if( strTmp.GetLength() && (nPos == (nLen-1)) )
			{
				strTmp = strTmp + CDuiString(_T("00"));
				m_pOwner->SetText(strTmp);
			}
			if( 3 <= (nLen-nPos-1) )
			{
				CDuiString strR;
#ifdef _UNICODE
				strR.Format(_T("%0.2f"),_wtof( strTmp.GetData() ));
#else
				strR.Format(_T("%0.2f"),atof( strTmp.GetData() ));
#endif
				m_pOwner->SetText(strR);
			}
		}
		m_pOwner->GetManager()->SendNotify(m_pOwner, DUI_MSGTYPE_KILLFOCUS_EW, wParam, lParam);
	...
	LRESULT CEditWnd::OnEditChanged(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		m_pOwner->GetManager()->SendNotify(m_pOwner, DUI_MSGTYPE_TEXTCHANGED,0,(LPARAM)pstr);
		m_pOwner->OnEditChanged(pstr);   //add by linwf at 2012-08-27

		TEventUI tEvent = { 0 };
		tEvent.pSender = m_pOwner;
		tEvent.Type = UIEVENT_EDITCHANGED;
		m_pOwner->OnEdit_Changed(&tEvent);
		return 0;
	}
	...
	CEditUI::CEditUI() : m_pWindow(NULL), m_uMaxChar(255), m_bReadOnly(false), 
		...
		m_dwEditbkColor(0xFFFFFFFF), m_iWindowStyls(0),m_pChecker(0),
		m_bEnableFloat(false),m_dwHotBorderColor(0)
	{
	...
	void CEditUI::EnableFloat(bool bT)
	{
		m_bEnableFloat = bT;
	}
	bool CEditUI::GetEnableFloat()
	{
		return m_bEnableFloat;
	}
	void CEditUI::SetChecker( CheckInput *pChecker )
	{
		m_pChecker = pChecker;
	}
	CheckInput*	CEditUI::GetChecker()
	{
		return m_pChecker;
	}
	...
	void CEditUI::SetPos(RECT rc)
	{
		...
		if( m_pWindow != NULL ) {
			m_pWindow->CalPos();
			::SetWindowPos(m_pWindow->GetHWND(),NULL,m_pWindow->m_rcCal.left,
				m_pWindow->m_rcCal.top,m_pWindow->m_rcCal.GetWidth(), 
				m_pWindow->m_rcCal.GetHeight(),SWP_NOZORDER | SWP_NOACTIVATE);        
		}
	}
	...
	{
		if( ((m_uButtonState & UISTATE_HOT) != 0) && m_dwHotBorderColor )
		{
	...
	//add by linwf at 2012-08-27
	LRESULT CEditUI::OnEditChanged(LPTSTR text)
	{
		return 0;
	}

	//add by linwf at 2012-08-27
	LRESULT CEditUI::OnEditKeyDown(WPARAM key)
	{
		return 0;
	}
}