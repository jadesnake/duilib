...
namespace DuiLib
{
	...
	void CLabelUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if( _tcscmp(pstrName,_T("suggest"))==0 )
		{
			m_nSuggest = _ttoi(pstrValue);
		}
	...
	void CLabelUI::PaintText(HDC hDC)
	{
		...
		else
		{
			...
			if ((m_uTextStyle & DT_END_ELLIPSIS) == DT_END_ELLIPSIS)
			{
