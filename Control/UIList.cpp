...
//
//Add by jade
CListBodyUI*	CListUI::NewListBodyUI()
{
	return new CListBodyUI(this);
}
CListHeaderUI*	CListUI::NewListHeaderUI()
{
	return new CListHeaderUI;
}
void CListUI::SetManager(CPaintManagerUI* pManager,CControlUI* pParent,bool bInit)
{
	__super::SetManager(pManager,pParent,bInit);
	CContainerUI*  pBody = GetList();
	CListHeaderUI* pHeader = GetHeader();
	if( NULL == pBody->GetParent() )
	{
		CVerticalLayoutUI::Add(GetList());
	}
	if( pHeader )
	{
		if( NULL == pHeader->GetParent() )
			Add(pHeader);
	}
}

CListUI::CListUI() 
	: m_pCallback(NULL), m_bScrollSelect(false), m_iCurSel(-1), m_iExpandedItem(-1),
	m_pHeader(NULL),m_pList(NULL),m_bAutoFixedLst(false)
{
}
CListUI::~CListUI()
{
...
PVOID CListUI::GetInterface(LPCTSTR pstrName)
{
...
{
	CListUI *pT = const_cast<CListUI*>(this);
    return pT->GetList()->GetItemAt(iIndex);
}
...
{	
    ...
    if( _tcsstr(pControl->GetClass(), _T("ListHeaderItemUI")) != NULL ) return pT->GetHeader()->GetItemIndex(pControl);

    return pT->GetList()->GetItemIndex(pControl);
}
...
bool CListUI::SetItemIndex(CControlUI* pControl, int iIndex)
{
    ...
    // We also need to recognize header sub-items
    if( _tcsstr(pControl->GetClass(), _T("ListHeaderItemUI")) != NULL ) return GetHeader()->SetItemIndex(pControl, iIndex);

    int iOrginIndex = GetList()->GetItemIndex(pControl);
    ...
    if( !GetList()->SetItemIndex(pControl, iIndex) ) return false;
    int iMinIndex = min(iOrginIndex, iIndex);
    int iMaxIndex = max(iOrginIndex, iIndex);
    for(int i = iMinIndex; i < iMaxIndex + 1; ++i) {
        CControlUI* p = GetList()->GetItemAt(i);
...
bool CListUI::Add(CControlUI* pControl)
{
    ...
    if( pControl->GetInterface(_T("ListHeader")) != NULL ) {
        if( GetHeader() != pControl && GetHeader()->GetCount() == 0 ) {
            CVerticalLayoutUI::Remove(GetHeader());
            m_pHeader = static_cast<CListHeaderUI*>(pControl);
        }
        m_ListInfo.nColumns = MIN(GetHeader()->GetCount(), UILIST_MAX_COLUMNS);
        return CVerticalLayoutUI::AddAt(pControl, 0);
    }
    // We also need to recognize header sub-items
    if( _tcsstr(pControl->GetClass(), _T("ListHeaderItemUI")) != NULL ) {
        bool ret = GetHeader()->Add(pControl);
        m_ListInfo.nColumns = MIN(GetHeader()->GetCount(), UILIST_MAX_COLUMNS);
        return ret;
    }
...
bool CListUI::AddAt(CControlUI* pControl, int iIndex)
{
    ...
    // We also need to recognize header sub-items
    if( _tcsstr(pControl->GetClass(), _T("ListHeaderItemUI")) != NULL ) {
        bool ret = GetHeader()->AddAt(pControl, iIndex);
        m_ListInfo.nColumns = MIN(GetHeader()->GetCount(), UILIST_MAX_COLUMNS);
        return ret;
    }
    if (!GetList()->AddAt(pControl, iIndex)) return false;
    ...
    for(int i = iIndex + 1; i < GetList()->GetCount(); ++i) {
        CControlUI* p = GetList()->GetItemAt(i);
...
bool CListUI::Remove(CControlUI* pControl)
{
    ...
    // We also need to recognize header sub-items
    if( _tcsstr(pControl->GetClass(), _T("ListHeaderItemUI")) != NULL ) return GetHeader()->Remove(pControl);

    int iIndex = GetList()->GetItemIndex(pControl);
    if (iIndex == -1) return false;

    if (!GetList()->RemoveAt(iIndex)) return false;

    for(int i = iIndex; i < GetList()->GetCount(); ++i) {
        CControlUI* p = GetList()->GetItemAt(i);
        IListItemUI* pListItem = static_cast<IListItemUI*>(p->GetInterface(_T("ListItem")));
        if( pListItem != NULL ) {
            pListItem->SetIndex(i);
...
void CListUI::SetPos(RECT rc)
{
    ...
    if( !GetHeader()->IsVisible() ) {
        for( int it = 0; it < GetHeader()->GetCount(); it++ ) {
            static_cast<CControlUI*>(GetHeader()->GetItemAt(it))->SetInternVisible(true);
        }
        GetHeader()->SetPos(CDuiRect(rc.left, 0, rc.right, 0));
    }
    int iOffset = GetList()->GetScrollPos().cx;
    for( int i = 0; i < m_ListInfo.nColumns; i++ ) {
        CControlUI* pControl = static_cast<CControlUI*>(GetHeader()->GetItemAt(i));
    ...
    if( !GetHeader()->IsVisible() ) 
	{
        for( int it = 0; it < GetHeader()->GetCount(); it++ ) 
		{
            static_cast<CControlUI*>(GetHeader()->GetItemAt(it))->SetInternVisible(false);
        }
    }
	bool bCalFixed = false;
	if( GetHeader()->IsVisible() && GetList()->IsVisible() )
	{
		if( GetHeader()->GetFixedHeight() && GetCount() )
			bCalFixed = true;
	}
	if( m_bAutoFixedLst )
	{
		...
		{
			if( GetHeader()->IsVisible() )
				nHeaderH = GetHeader()->GetFixedHeight();
		}
		...
		if( pB && pE && (pB != pE) )
		{
			if( pB->IsVisible() && pE->IsVisible() )
			{
				RECT rcB = pB->GetPos();
				RECT rcE = pE->GetPos();
				long lH  = nBorder+rcE.bottom-rcB.top+pE->GetPadding().bottom+pB->GetPadding().top+nHeaderH;
				if( GetFixedHeight() != lH )
					SetFixedHeight(lH);
			}
			else if( nHeaderH != GetFixedHeight() )
			{
				m_rcItem.bottom = m_rcItem.top + nHeaderH;
				SetFixedHeight(nHeaderH);
			}
		}
		else if( pB )
		{
			...
			{
				RECT rcB = pB->GetPos();
				long lH  = nBorder+rcB.bottom-rcB.top+pB->GetPadding().bottom+pB->GetPadding().top+nHeaderH;
				if( GetFixedHeight() != lH )
					SetFixedHeight(lH);
			}
			else if( nHeaderH != GetFixedHeight() )
		...
		else if( pE )
		{
			...
			{
				RECT rcE = pE->GetPos();
				long lH  = nBorder+rcE.bottom-rcE.top+pE->GetPadding().bottom+pE->GetPadding().top+nHeaderH;
				if( GetFixedHeight() != lH )
					SetFixedHeight(lH);
			}
			else if( nHeaderH != GetFixedHeight() )
...
void CListUI::DoEvent(TEventUI& event)
{
    ...
    case UIEVENT_SCROLLWHEEL:
        {
			//fixed by jade 修正事件丢失
            switch( LOWORD(event.wParam) ) {
            case SB_LINEUP:
                if( m_bScrollSelect )
					SelectItem(FindSelectable(m_iCurSel - 1, false), true);
				else if( GetVerticalScrollBar() )
				{
				...
				else if( GetHorizontalScrollBar() )
				{
...
{
	if( m_pHeader == 0)
		m_pHeader = NewListHeaderUI();
	return m_pHeader;
}

CContainerUI* CListUI::GetList()
{
	if( m_pList == 0)
		m_pList = NewListBodyUI();
	if( m_pList == 0 )
		m_pList = CListUI::NewListBodyUI();
	return m_pList;
}
...
{
	CListUI *pT = const_cast<CListUI*>(this);
    return pT->GetList()->GetChildPadding();
}

void CListUI::SetChildPadding(int iPadding)
{
    GetList()->SetChildPadding(iPadding);
}
...
void CListUI::SetHotBorderColor(DWORD dwBkColor)
{
	m_ListInfo.dwHotBorderColor = dwBkColor;
	Invalidate();
}
void CListUI::SetBorderSize(DWORD dwV)
{
	m_ListInfo.dwBorderSize = dwV;
	Invalidate();
}
void CListUI::SetSelectedBorderColor(DWORD dwBkColor)
{
	m_ListInfo.dwSelectedBorderColor = dwBkColor;
	Invalidate();
}
...
DWORD CListUI::GetItemHotBorderColor(void) const
{
	return m_ListInfo.dwHotBorderColor;
}
DWORD CListUI::GetItemBorderSize(void) const
{
	return m_ListInfo.dwBorderSize;
}
DWORD CListUI::GetItemSelectedBorderColor(void) const
{
	return m_ListInfo.dwSelectedBorderColor;
}
...
void CListUI::EnsureVisible(int iIndex)
{
    ...
    RECT rcItem = GetList()->GetItemAt(iIndex)->GetPos();
    RECT rcList = GetList()->GetPos();
    RECT rcListInset = GetList()->GetInset();
    ...
    CScrollBarUI* pHorizontalScrollBar = GetList()->GetHorizontalScrollBar();
    if( pHorizontalScrollBar && pHorizontalScrollBar->IsVisible() ) rcList.bottom -= pHorizontalScrollBar->GetFixedHeight();

    int iPos = GetList()->GetScrollPos().cy;
...
{
    if( dx == 0 && dy == 0 ) return;
    SIZE sz = GetList()->GetScrollPos();
    GetList()->SetScrollPos(CSize(sz.cx + dx, sz.cy + dy));
}
...
void CListUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("itemhotbordercolor")) == 0 )	//悬停时候边框颜色
	{
		if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
		LPTSTR pstr = NULL;
		SetHotBorderColor(_tcstoul(pstrValue, &pstr, 16));
	}
	...
	else if( _tcscmp(pstrName, _T("selectedbordercolor")) == 0 )
	{
		if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
		LPTSTR pstr = NULL;
		SetSelectedBorderColor(_tcstoul(pstrValue, &pstr, 16));
	}
	...
	else if( _tcscmp(pstrName, _T("autofixed")) == 0 )
	{
		if( _tcscmp(pstrValue, _T("true")) == 0 )
		{
			m_bAutoFixedLst = true;
			m_bAutoFixed	= false;		//防止死循环
		}
		else if( _tcscmp(pstrValue, _T("false")) == 0 )
			m_bAutoFixedLst = false;
	}
...
{
	CListUI *pT = const_cast<CListUI*>(this);
    return pT->GetList()->GetScrollPos();
}

SIZE CListUI::GetScrollRange() const
{
	CListUI *pT = const_cast<CListUI*>(this);
    return pT->GetList()->GetScrollRange();
}

void CListUI::SetScrollPos(SIZE szPos)
{
    GetList()->SetScrollPos(szPos);
}
...
{
    GetList()->EnableScrollBar(bEnableVertical, bEnableHorizontal);
}

CScrollBarUI* CListUI::GetVerticalScrollBar() const
{
	CListUI *pT = const_cast<CListUI*>(this);
	return pT->GetList()->GetVerticalScrollBar();
}

CScrollBarUI* CListUI::GetHorizontalScrollBar() const
{
	CListUI *pT = const_cast<CListUI*>(this);
	return pT->GetList()->GetHorizontalScrollBar();
}

BOOL CListUI::SortItems(PULVCompareFunc pfnCompare, UINT_PTR dwData)
{
	CContainerUI* pCtrl = GetList();
	if( pCtrl->GetInterface(_T("ListBody")) )
	{
		CListBodyUI *ListBody = static_cast<CListBodyUI*>(pCtrl);
		ListBody->SortItems(pfnCompare, dwData);
	}
	return FALSE;
}
/////////////////////////////////////////////////////////////////////////////////////
...
void CListContainerElementUI::Invalidate()
{
    ...
    if( GetParent() ) {
        ...
        if( pParentContainer ) {
			...
			{
				//if the container used outer scrollbar, the paint area should not be trimmed
				if( pParentContainer->UsedOuterVScrollBar() == false ) 
				{
...
void CListContainerElementUI::DrawItemBk(HDC hDC, const RECT& rcItem)
{
	...
	if( pInfo->dwBorderSize && ((m_uButtonState&UISTATE_HOT)!= 0) )
	{
		CRenderEngine::DrawRoundRect(hDC,m_rcItem,pInfo->dwBorderSize,
			0,0,GetAdjustColor(pInfo->dwHotBorderColor));
	}
	if( IsSelected() && pInfo->dwBorderSize ) 
	{
		CRenderEngine::DrawRoundRect(hDC,m_rcItem,pInfo->dwBorderSize,
			0,0,GetAdjustColor(pInfo->dwSelectedBorderColor));
	}
    if( !IsEnabled() ) {
        if( !pInfo->sDisabledImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)pInfo->sDisabledImage) ) pInfo->sDisabledImage.Empty();
