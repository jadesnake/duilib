...
{
    //20150302 shidd add 增加bIsScrollBar定义，表示鼠标左键按下时，是否点击了滚动条
	static bool bIsScrollBar = true;
	if( uMsg == WM_CREATE ) {
        m_pm.Init(m_hWnd);
	...
	//20150302 shidd add 增加鼠标左键按下事件，记录是否点击了滚动条
        ...
        ::ScreenToClient(m_pm.GetPaintWindow(), &pt);
        CControlUI* pControl = m_pm.FindControl(pt);
		//20150302 shidd mod 拖动滚动条放开之后，如果落点在下拉框之内的不杀死焦点
        //if( pControl && _tcscmp(pControl->GetClass(), _T("ScrollBarUI")) != 0 ) PostMessage(WM_KILLFOCUS);
		if( pControl && _tcscmp(pControl->GetClass(), _T("ScrollBarUI")) != 0 && !bIsScrollBar ) PostMessage(WM_KILLFOCUS);
    }
...
{
    //20150302 shidd 注释以下一行代码，支持使用鼠标滚轮和上下键操作时下拉框不会消失
